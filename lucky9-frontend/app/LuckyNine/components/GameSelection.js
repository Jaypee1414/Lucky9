...
function GameSelection({ value, bet, isBanker,gamePhase , setIsPlayerCoin,isPlayerCoin}) {
  ...
  return (
    <div className="absolute bottom-0 left-0">
      ...
      {isBanker || gamePhase === "countdown" || gamePhase === "selectBanker" ? null : (   <div className="flex flex-row">
          <div className="relative w-auto h-auto">
            <button onClick={() => handleBet(2000)}
            disabled={gamePhase === "drawPhase" || isBanker || gamePhase === "results" || isPlayerCoin < 2000 }  
            className={`h-20 w-auto cursor-pointer ${gamePhase === "drawPhase" || isBanker || gamePhase === "results" || isPlayerCoin < 2000  ? "opacity-65 cursor-not-allowed" : ""}`}
            >
            <Image
          ...
          <div className="relative w-auto h-auto ">
            <button onClick={() => handleBet(value)}
              disabled={gamePhase === "drawPhase" || isBanker || gamePhase === "results" || isPlayerCoin < value}  
              className={`h-20 w-auto cursor-pointer ${gamePhase === "drawPhase" || isBanker || gamePhase === "results" || isPlayerCoin < value ? "opacity-65 cursor-not-allowed" : ""}`}
              >
            <Image
              src="/image/ButtonGameBackground.svg"
              width={40}
        ...
        {/* right Side */}
        {gamePhase !== "countdown" &&  gamePhase !== "selectBanker" ? (  <div className="flex flex-row h-full items-start justify-start">
